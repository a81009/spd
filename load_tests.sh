...
create_put_urls_file() {
    ...
    # Adiciona URLs de PUT com valores diferentes
    for i in $(seq 1 $NUM_KEYS); do
        # Formato: URL POST dados
        # Preciso escapar as aspas para que o JSON seja formatado corretamente
        echo "$API_URL/kv POST {\"data\":{\"key\":\"test_key_$i\",\"value\":\"test_value_$i\"}}" >> "$filename"
    done
