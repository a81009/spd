# PowerShell script for GET operations on KV store
...
$GET_TPS = Read-Input -prompt "GET ops/s" -default "50"
$TEST_NAME = Read-Input -prompt "Test name" -default "kv-gets-test"
...
$GET_TPM = To-Min -val $GET_TPS  # This will be requests per minute

# Calculate fixed values for test plan - MODIFIED to make GET_TPS be per user
[int]$requestsPerUser = [int]$DURATION * [int]$GET_TPS  # Each user should make this many requests
...
[int]$delayMs = [int]([Math]::Floor(1000 / $GET_TPS))
...
Write-Host "GET TPS per user.....: $GET_TPS"
...
# Script to get a random key for GET operation
$getKeyScript = @"
...
# Script to track cache/DB hits
$cacheCheckScript = @"
// Check if the response indicates a cache hit
...
return isCache ? "CACHE_HIT" : "DB_HIT";
"@
...
# Make sure we escape all scripts properly for XML
...
$cacheCheckScript = $cacheCheckScript -replace '&', '&amp;' -replace '<', '&lt;' -replace '>', '&gt;' -replace '"', '&quot;' -replace "'", '&apos;'
    ...
    props.put("get_total", String.valueOf(count));
        ...
        <hashTree>
          <JSR223PostProcessor guiclass="TestBeanGUI" testclass="JSR223PostProcessor" testname="Check Cache Hit">
            ...
            <stringProp name="filename"></stringProp>
            <stringProp name="cacheKey">true</stringProp>
            <stringProp name="script">$cacheCheckScript</stringProp>
          </JSR223PostProcessor>
          <hashTree/>
        </hashTree>
      </hashTree>
...
# Modified summary display to match the other scripts
Write-Host "`n===== Summary ====="
Write-Host "Total GET operations...: $total"
...
# Store the stats report information to display later if requested
$statsFiles = Get-ChildItem -Path $STATS_DIR -Filter "get-stats-report-$TIMESTAMP.txt" -ErrorAction SilentlyContinue
if ($statsFiles.Count -gt 0) {
    $statsFile = $statsFiles[0].FullName
    
    # Display the stats report in the same format as run-puts.ps1
